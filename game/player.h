...
#define PLAYER_HARVEST_DIST 110
...
public:
    ...
    void update(float time,vector<ObjectStatic*> vector_objs_solid,Camera *camera);
    //### draw ###
    void draw(RenderWindow *window,Camera *camera);

    //### check collision ###
    bool checkCollision(float dirx,float diry,vector<ObjectStatic*> vector_objs,Camera *camera);
