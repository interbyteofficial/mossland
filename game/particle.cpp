...
//###
void ParticleEmmiter::draw(RenderWindow *window,Camera *camera)
{
    //for each particle
    for(auto it=this->vector_particles.begin(); it!=this->vector_particles.end(); it++)
    {
        if(!(*it)->remove){
            ...
            this->rect->setPosition(Vector2f((*it)->x-camera->x,(*it)->y-camera->y));
            this->rect->setSize(Vector2f(this->w,this->h));
...
//### add some value to current position
void ParticleEmmiter::addPosition(int x,int y){
    this->x+=x;
    this->y+=y;
}
//###
//### getters | get some value from variable of class
//###
int ParticleEmmiter::getCount(){
    return this->vector_particles.size();
}
//################
//###          ###
//### Particle ###
